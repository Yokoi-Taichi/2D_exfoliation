...
def OneCH2TreeCH(image_1ch, labels=2):
    ...
    step = 255/(labels-1)
    color = np.array([
        [int(i*step), int(i*step), int(i*step)] for i in range(labels)]) #ラベルの階数のカラーパレット
...
def savefigs():
    ...
    cv2.imwrite(
        path+"after_binalhighlight.png",
        highlightbinalimage(img_after, after_clust.binalImage_3ch, [0, 255, 0]))
    ...
    cv2.imwrite(
        path+"before_binalhighlight.png",
        highlightbinalimage(img_before, before_clust.binalImage_3ch, [0, 255, 0]))
...
class ObjectDetecting():
    ...
    def Detectedobjects(self, highlightedImage = None):
        ...
        for i in range(nlabels): #重心に×印
            img_out = cv2.drawMarker(
                img_out, tuple(centroids[i].astype(np.int)), (0, 0, 255),
                markerType=cv2.MARKER_TILTED_CROSS, markerSize=30, thickness=2)
...
class templatematching():
    ...
    def tempmatch(self):
        ...
        after = self.after
        template = after[target_stats[cv2.CC_STAT_TOP]:\
                         target_stats[cv2.CC_STAT_TOP]+target_stats[cv2.CC_STAT_HEIGHT],\
                         target_stats[cv2.CC_STAT_LEFT]:\
                         target_stats[cv2.CC_STAT_LEFT]+target_stats[cv2.CC_STAT_WIDTH]]
        ...
        #matchした場所に枠を表示
        img_ext = cv2.rectangle(img_ext, min_loc,
...
class DifferenceDetection(templatematching):
    def __init__(self, 
                 before_formatch, after_formatch, target_stats,
                 before_original=None, after_original=None):
    ...
    def trimBefore(self, before_image):
        ...
        b_holiz = [0, self.before_original.shape[0],\
                   self.move_list[1], self.after_original.shape[0]+self.move_list[1]]
        b_vart = [0, self.before_original.shape[1],\
                  self.move_list[0], self.after_original.shape[1]+self.move_list[0]]
        b_holiz.sort()
        b_vart.sort()
    ...
    def trimAfter(self, after_image):
        ...
        a_holiz = [-1*self.move_list[1], 0,\
                   self.before_original.shape[0]-self.move_list[1],self.after_original.shape[0]]
        a_vart = [-1*self.move_list[0], 0,\
                  self.before_original.shape[1]-self.move_list[0], self.after_original.shape[1]]
        a_holiz.sort()
        a_vart.sort()
...
if __name__ == '__main__':
    ...
    img_before = cv2.resize(img_before,
                            (int(shape_before[1]*expantion), int(shape_before[0]*expantion)))
    ...
    matching = DifferenceDetection(before_clust.binalImage_3ch, after_clust.binalImage_3ch,
                                   after_flakes.dictionally["stats"][1], img_before, img_after)
